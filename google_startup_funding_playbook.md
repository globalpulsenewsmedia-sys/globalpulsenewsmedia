...
```mermaid
graph TD
    A[Prepare Domains & Email] --> B[Create GCP Account]
    B --> C[Set up GCP Billing Profile]
    C --> D[Go to cloud.google.com/startup]
    D --> E[Select 'AI Startup' Track]
    E --> F[Fill Out Copy-Paste Prompts]
    F --> G[Submit and Await Verification Email]
    G --> H[Claim Credits & Connect to Billing]
```
